...
public class Assignment_EnemyDetector : MonoBehaviour
{
    ...
    {
        Vector3 targeted = enemy.position - transform.position;
        if(targeted.magnitude > detectionRange)
            return false;
        Vector3 targetedNorm = targeted.normalized;
        float dot = Vector3.Dot(targetedNorm, transform.forward);

        float HalfFOV = Mathf.Cos(detectionFOV * 0.5f * Mathf.Deg2Rad);
